...
const PIE_COLORS = ['hsl(217, 91%, 50%)', 'hsl(160, 84%, 39%)', 'hsl(38, 92%, 50%)'];
...
export const DashboardPage = () => {
  ...
  const growthData = [
    ...
    { month: 'Jan', users: stats?.totalUsers || 38, institutions: stats?.totalInstitutions || 12 },
  ];

  const roleDistribution = [
    { name: 'Doctors', value: stats?.usersByRole?.Doctor || 0 },
    { name: 'Health Workers', value: stats?.usersByRole?.['Health Worker'] || 0 },
    ...
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-6">
      ...
      <motion.div variants={item} className="flex items-end justify-between">
        <div>
          ...
          <p className="text-slate-600 mt-1">Digital Health System administration overview</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        {/* Line Chart */}
        <motion.div variants={item} className="lg:col-span-2 rounded-2xl border border-slate-200 bg-white p-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h2 className="text-sm font-semibold text-slate-900">System Growth</h2>
              <p className="text-xs text-slate-500 mt-0.5">Users & institutions over time</p>
            </div>
          ...
          <ResponsiveContainer width="100%" height={260}>
            <LineChart data={growthData}>
              ...
              />
              <Line type="monotone" dataKey="users" stroke="hsl(217, 91%, 50%)" strokeWidth={2.5} dot={false} />
              <Line
                type="monotone"
        ...
        {/* Donut Chart */}
        <motion.div variants={item} className="rounded-2xl border border-slate-200 bg-white p-6 flex flex-col">
          <div className="mb-4">
            <h2 className="text-sm font-semibold text-slate-900">User Role Distribution</h2>
            <p className="text-xs text-slate-500 mt-0.5">Across all institutions</p>
          </div>
      ...
      {/* Recent Users Table */}
      <motion.div variants={item} className="rounded-2xl border border-slate-200 bg-white overflow-hidden">
            ...
            <tbody>
              {recentUsers.map((user, i) => (
                ...
                >
                  ...
                  <td className="px-6 py-3.5 text-slate-600">{user.institution?.name}</td>
                  <td className="px-6 py-3.5">
                    <span
